...
# gtsm boundary
modelfile=os.path.join(path1,'model_runs','cartesius_runs','test_boundary_runs','gtsmboundaryoutput','canada_model_0000_his.nc')
locindex=(np.linspace(0,474,475)).astype(int)
moddata=readdata.readmodel(modelfile,locindex)

# #reading fes data on boundary.
fesboundfile=os.path.join(path1,'FESCanada','fes10km_5e-2_obs.nc')
fesdata=readdata.readfes(fesboundfile,locindex)

# reading gtsm run boundary data.
gtsmboundfile=os.path.join(path1,'gtsm_runs','Jan2013v4.1','gtsm_model_0000_his.nc')
...
#%%
plttinstant(moddata,fesdata,gtsmdata,j,locindex)
